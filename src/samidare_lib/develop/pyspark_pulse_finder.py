...
import samidare_lib.develop.binary_dumper_version3 as bd
...
def common_options(func):
    ...
    @click.option('--tsoffset', '-tsof'  , type=float, default=0., help='timestamp offset')
...
@common_options
def main(clock, tsoffset, rise, fall, pren, postn, minlen, maxevt, checkts, checkpf, duration, save, refch, file, dir):
    ...
    df_plot = (
        df_pulses
        .withColumn("t0_ms", (F.element_at("pulse_timestamp", 1).cast("double") )/F.lit(c_timestamp) - F.lit(tsoffset)  )
        # .withColumn("t0_ms", F.element_at("pulse_timestamp", 1)/F.lit(32.00) * F.lit(3.125) / F.lit(1e6))
        .where(F.col("t0_ms").isNotNull())
